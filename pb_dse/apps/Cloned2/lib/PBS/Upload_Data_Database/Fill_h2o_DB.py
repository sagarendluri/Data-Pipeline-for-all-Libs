...
def h2o_DB_upload(Accuracy,X_train,X_test,rmse,mse,rmsle,importances,grid,estimator,cm,target,model_file_name,model,dname,
                  config_object_user,user_defined_terminology,sample_type ,description ,uom_type,analysis_id,db_name,result):
#     try:
        ...
        mySql_insert_query = """INSERT INTO Phenotype_model_info(model_file_path,
                                            ...
                                            dataset_name, request_payload,performance,Phenotypes_id_id ,analysis_name)
        ...
        if str(user_defined_terminology) in res:
            ...
            recordTuple = (model_file_name, json.dumps(cm),
                            ...
                            json.dumps(cr),res[d],analysis_id
            )
            ...
            recordTuple = (model_file_name, json.dumps(cm),
                            ...
                            json.dumps(esti),
                            dname, json.dumps(request_payload_data),
                            json.dumps(cr),phe_id_id,analysis_id
            )
            cursor.execute(mySql_insert_query, recordTuple)
            conn.commit()
#     except:
