...
class Model():
    def __init__(self,*args,**kwargs):
        ...
        self.config_object.read(self.ini)
        self.userinfo = self.config_object["IRRIGENOTYPES"]
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument("--phenome_dataset", help="enter dataset name", type = str)
        ...
        if extension == '.csv':
            ...
            try:
                ...
                # vcf file is ready #
                print(sum_df.head())
                object_key = (', '.join(["%s" % pname]))
                path = 's3://{}:{}@{}/{}'.format(access_key_id, secret_access_key, bucket_name, object_key)
                phe  = pd.read_csv(smart_open(path))
                new_phe = phe.set_index('Line')
                ...
                final1 = final1.fillna(final1.mean())
                df1 = final1.sort_values(i)
                print(df1)
                model_instance = Model(df1,i,ini = ini,sklearn = sklearn,ai= ai)
                model_instance.dtypes_handliing()
                print(model_instance.handiling_categorical())
