...
#         try:
            ...
            dname,target,sklearn,algorithm,config_object_user,ai,model_file,predict,phenome_data,user_defined_terminology,sample_type,description,uom_type,all_M,d_col,index,to_csv_name,min_depth,max_depth,min_samples_split,n_estimators_start,n_estimators_stop,n_neighbors,xgb_objective,xgb_learning_rate,xgb_max_depth,xgb_min_child_weight,svm_C,svm_gamma,svm_kernel,default,cut_off,output,analysis_id,db_name,N_features,pca,learning_rate_init ,max_iter, hidden_layer_sizes, activation,alpha,early_stopping=args(pd_df,df)
#             saveout = sys.stdout
#             fsock = open(output, 'w')
#             sys.stdout = fsock
            got_to_s(dname,target,sklearn,algorithm,config_object_user,ai ,model_file,predict,phenome_data,user_defined_terminology,sample_type,description,uom_type,all_M,d_col,index,to_csv_name,min_depth,max_depth,min_samples_split,n_estimators_start,n_estimators_stop,n_neighbors,xgb_objective,xgb_learning_rate,xgb_max_depth,xgb_min_child_weight,svm_C,svm_gamma,svm_kernel,default,analysis_id,db_name,N_features,pca,learning_rate_init ,max_iter, hidden_layer_sizes, activation,alpha,early_stopping,cut_off)
#             sys.stdout = saveout
#             fsock.close()
#             f = open(output, "r")
#             df = f.read()
...
class Pyspark_pipeline(APIView):
    def post(self,data):
        ...
        json_args.save()
        pd_df = pd.DataFrame.from_dict(df)
        dname,target,DTClassifier,RFClassifier,config_object_user,model_file,predict,phenome_data,       user_defined_terminology,sample_type,description,uom_type,all_M,d_col,index,to_csv_name,Classification_or_Regression,analysis_id,output=argss(pd_df,df)
        saveout = sys.stdout
        fsock = open(output, 'w')
        sys.stdout = fsock
