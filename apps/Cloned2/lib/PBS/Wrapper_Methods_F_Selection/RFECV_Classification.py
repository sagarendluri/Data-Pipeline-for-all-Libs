...
import joblib
from apps.Cloned2.lib.PBS.Wrapper_Methods_F_Selection.Wrapper_Methods_Models.Classification_Models import \
    RFECV_RF_Classifier, RFECV_XGB_Classifier
...
from apps.Cloned2.lib.PBS.SKlearn_grids.User_Grids import User_RandomForest_Classifier_grids, \
    User_XGBoost_Classifier_grids
import warnings

warnings.filterwarnings('ignore')


class ALL_CLS_Modeling:
    def __init__(self, data_sorted, y, i, types, dname, config_object_user,
                 ...
                 svm_kernel,
                 default,
                 db_name, N_features, learning_rate_init, max_iter, hidden_layer_sizes, activation, alpha,
    ...
    def Building_Models_Cls(self):
        ...
        def modelGeneratorFun():
            yield RFECV_RF_Classifier, self.RandomForest_Classifier_grids
            yield RFECV_XGB_Classifier, self.XGBoost_Classifier_grids

        for model, grids in modelGeneratorFun():
            #                 try:
            model_ = "RFECV_Regressor"
            train_accuracy, test_accuracy, X_train, X_test, y_test, random_best, importances, grid, estimator, l, cm, target, model_file_name = model(
                X_train, y_train, X_test, y_test, grids, target)
            DB_upload(train_accuracy, test_accuracy, X_train, X_test, y_test, random_best,
                      importances, grid, estimator, l, "None", target, model_file_name, model_, dname,
                      config_object_user,
                      user_defined_terminology, sample_type, description, uom_type, analysis_id, db_name, dataset_type,
                      result)
