        ...
        for i in range(len(correlation_matrix .columns)):
            for j in range(i):
                if abs(correlation_matrix.iloc[i, j]) > cut_off:
                    ...
                    dropped_correlated_features.append({"feature_name":colname,"correlation_value":abs(correlation_matrix.iloc[i, j])})
                    correlated_features.add(colname)
        X_train.drop(labels=correlated_features, axis=1, inplace=True)
        X_test.drop(labels=correlated_features, axis=1, inplace=True)
        seen = set()
        result = []
        for dic in dropped_correlated_features:
            key = (dic['feature_name'])
            if key in seen:
                continue
            result.append(dic)
            seen.add(key)
