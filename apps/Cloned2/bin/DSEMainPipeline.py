...
import sys
from apps.Cloned2.lib.PBS.Outliers.Reg_Class import Outlier_R_and_C_Models
...
from apps.Cloned2.lib.PBS.Predictions.Predict_Reg_class_selection import Csv_predicts
from apps.Cloned2.lib.PBS.VIF.Models import VIF_Models
...
def Decision_M(dname, i, sklearn, algorithm,
               config_object_user, ai, model_file, predict, phenome_data, user_defined_terminology, sample_type,
               description, uom_type, all_M, d_col, index, to_csv_name, min_depth, max_depth, min_samples_split,
               n_estimators_start, n_estimators_stop, n_neighbors, xgb_objective, xgb_learning_rate,
               xgb_max_depth, xgb_min_child_weight, svm_C, svm_gamma, svm_kernel, default, analysis_id, db_name,
               N_features,filter_M, pca,
               learning_rate_init, max_iter, hidden_layer_sizes, activation, alpha, early_stopping, dataset_type,
               test_size, Samples, vifNo, VIF, Outliers, cut_off=0.8):
    #     try:
    i = i
    dname = dname
    ...
    if "RFECV" == N_features():
        results = RFECV_Models(path, i, ini, dname, config_object_user,
                               user_defined_terminology, sample_type, description, uom_type, d_col, cut_off,
                               analysis_id, min_depth, max_depth, min_samples_split,
                               n_estimators_start, n_estimators_stop, n_neighbors, xgb_objective, xgb_learning_rate,
                               xgb_max_depth, xgb_min_child_weight, svm_C, svm_gamma, svm_kernel, default, db_name,
                               N_features, learning_rate_init, max_iter, hidden_layer_sizes, activation, alpha,
                               early_stopping, dataset_type, test_size)
        results.csv_Predicts()
    if "Outliers" == str(Outliers):
        results = Outlier_R_and_C_Models(path, i, ini, dname, config_object_user,
                                         user_defined_terminology, sample_type, description, uom_type, d_col, cut_off,
                                         analysis_id, min_depth, max_depth, min_samples_split,
                                         n_estimators_start, n_estimators_stop, n_neighbors, xgb_objective,
                                         xgb_learning_rate, xgb_max_depth, xgb_min_child_weight, svm_C, svm_gamma, svm_kernel, default,
                                         db_name,N_features, learning_rate_init, max_iter, hidden_layer_sizes, activation,
                                         alpha,early_stopping, dataset_type, test_size, vifNo, Samples)
        results.csv_Predicts()
    if "VIF" == VIF():
        results = RFECV_Models(path, i, ini, dname, config_object_user,
                               user_defined_terminology, sample_type, description, uom_type, d_col, cut_off,
                               ...
                               xgb_max_depth, xgb_min_child_weight, svm_C, svm_gamma, svm_kernel, default, db_name,
                               N_features, learning_rate_init, max_iter, hidden_layer_sizes, activation, alpha,
                               early_stopping, dataset_type, test_size, vifNo)
        results.csv_Predicts()
    elif "Filter_Methods" == filter_M:
        results = RFECV_Models(path, i, ini, dname, config_object_user,
                               user_defined_terminology, sample_type, description, uom_type, d_col, cut_off,
                               analysis_id, min_depth, max_depth, min_samples_split,
                               n_estimators_start, n_estimators_stop, n_neighbors, xgb_objective, xgb_learning_rate,
        ...
        results = Run_R_and_C_Models(path, i, ini, dname, config_object_user,
                                     user_defined_terminology, sample_type, description, uom_type, all_M, d_col,
                                     cut_off, access_key_id, secret_access_key, bucket_name, analysis_id, min_depth,
                                     max_depth, min_samples_split, n_estimators_start, n_estimators_stop, n_neighbors,
                                     xgb_objective, xgb_learning_rate, xgb_max_depth, xgb_min_child_weight, svm_C,
                                     svm_gamma, svm_kernel, default, db_name, learning_rate_init, max_iter,
                                     hidden_layer_sizes, activation, alpha, early_stopping, dataset_type, test_size,
                                     Samples)
        results.csv_Predicts()
    elif "Predict" == predict:
        results = Csv_predicts(path, pre_D, i, ini, dname, predict, model_file, config_object_user,
                               user_defined_terminology, sample_type, description, uom_type, d_col,
                               access_key_id, secret_access_key, bucket_name, analysis_id, db_name)
        results.csv_Predicts()
    else:
        results = Run_single_Class_Reg_Model(path, i, ini, algos, sklearn, ai, dname, config_object_user,
                                             user_defined_terminology, sample_type, description, uom_type, d_col,
                                             cut_off, access_key_id, secret_access_key, bucket_name, analysis_id,
                                             min_depth, max_depth, min_samples_split, n_estimators_start,
                                             n_estimators_stop, n_neighbors, xgb_objective, xgb_learning_rate,
                                             xgb_max_depth, xgb_min_child_weight, svm_C, svm_gamma, svm_kernel, default,
                                             db_name)
        results.csv_Predicts()
